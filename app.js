...
async function loadLiveGitHubProjects() {
    ...
    try {
        const response = await fetch (`https://github.com{GITHUB_USERNAME}/repos?sort=published&per_page=30`);
        //Safeguard against servor connection errors
        if {(!response.ok) throw new Error('API communication error occured, Please try again');
